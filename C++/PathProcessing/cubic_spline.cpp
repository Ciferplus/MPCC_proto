...
{
    // evaluation of spline at x
    int index;
    double x_i;
    double dx, dx2, dx3;
    // wrap input to data -> x data needs start at 0 and contain end point!!!
    x = unwrapInput(x);
    // compute index
    index = getIndex(x);
    // access previous points
    x_i = spline_data_.x_data(index);
    // compute diff to point and its powers
    dx = x-x_i;
    dx2 = dx*dx;
    dx3 = dx*dx2;
...
double CubicSpline::getDerivative(double x) const
{
    ...
    // identical to normal spline with y' = b + 2 c dx + 3 d dx^2
    int index;
    double x_i;
    double dx, dx2;
...
double CubicSpline::getSecondDerivative(double x) const
{
    ...
    // identical to normal spline with y'' = 2 c + 6 d dx
    int index;
    double x_i;
    double dx;
