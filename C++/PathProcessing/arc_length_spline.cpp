...
{ 
    params_ = SplineParams();
}

ArcLengthSpline::ArcLengthSpline(const SplineParams &params)
: params_(params)
{
}
...
void ArcLengthSpline::setData(const Eigen::VectorXd &X_in, const Eigen::VectorXd &Y_in)
{
    ...
    // compute arc length based on a piecewise linear approximation
...
void ArcLengthSpline::setRegularData(const Eigen::VectorXd &X_in, const Eigen::VectorXd &Y_in, const Eigen::VectorXd &s_in) {
    ...
    // x-y points are spaced such that they are very close to arc length parametrized
    if(X_in.size() == Y_in.size()){
        path_data_.X = X_in;
        path_data_.Y = Y_in;
        path_data_.n_points = X_in.size();
...
PathData ArcLengthSpline::resamplePath(const CubicSpline &initial_spline_x, const CubicSpline &initial_spline_y, const double total_arc_length) const
{
    ...
    // equally spaced between 0 and current length of path
...
double ArcLengthSpline::projectOnSpline(const SimpleState &state) const
{
    Eigen::Vector2d pos;
    pos(0) = state.X;
    pos(1) = state.Y;
    double s_guess = state.s;
    Eigen::Vector2d pos_path = getPosition(s_guess);

    double s_opt = s_guess;
    double dist = (pos-pos_path).norm();

    if (dist >= params_.max_dist_proj)
    {
